...
```{R}
rpart_classes2 <- predict(rpart_fit2, newdata = df2_test, type = "class")
rpart_acc2 <- mean(rpart_classes2 == df2_test$spam)
rpart_acc2
```
...
svm_fit2 <- svm(spam ~ ., data = df2_train, kernel = "radial")
```
Report the prediction accuracy on the test set.

```{R}
svm_classes2 <- predict(svm_fit2, newdata = df2_test)
svm_acc2 <- mean(svm_classes2 == df2_test$spam)
svm_acc2
```
...
```R
X_train <- model.matrix(spam ~ ., data = df2_train)[,-1]  # Remove the intercept
Y_train <- df2_train$spam

X_test <- model.matrix(spam ~ ., data = df2_test)[,-1]
...
X_train <- as.data.frame(apply(X_train, 2, normalize))
X_test <- as.data.frame(apply(X_test, 2, normalize))
...
nn_probs <- predict(nn_fit, X_test)

nn_classes <- ifelse(nn_probs > 0.5, 1, 0)

nn_accuracy <- mean(nn_classes == Y_test)
nn_accuracy
```
...
NN2 <- nn_module(
  initialize = function(p, q1, q2, q3, o){
    ...
    self$hidden2 <- nn_linear(q2, q1)
    self$hidden3 <- nn_linear(q3, q2)
    self$output <- nn_linear(o, q3)
    self$activation <- nn_functional$relu
  },
  forward = function(x){
    x %>% 
      self$hidden1() %>% 
